...
5. Build R8 release APKs:
   - `TV-mobile-arm64-v8a.apk`
   - `TV-leanback-armeabi-v7a.apk`
6. Verify both APK signatures, upload 30-day workflow artifacts, and publish a GitHub release with `SOURCE_REFS.json` and `SHA256SUMS.txt`.
...
The private key and passwords are never committed. Keeping this key stable allows a later autobuild APK to update an earlier autobuild installation.
